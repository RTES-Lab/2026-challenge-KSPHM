"""
Generate A-full specific figures for SR/0521/progress.md.
"""
import sys
import shutil
...
# Add rul/code to path so we can import from rul_th742_ensemble3.py
sys.path.append(str(Path("/data/home/ksphm/2026-challenge-KSPHM/User/SR/0521/rul/code")))
import rul_th742_ensemble3 as ensemble3

# ── Paths ──────────────────────────────────────────────────────────────────
BASE      = Path("/data/home/ksphm/2026-challenge-KSPHM")
OUT_FIGS  = BASE / "User/SR/0521/figures"
...
params, sigma, feat_w, grp_w, aux_w, aux_s, aux_features = ensemble3.load_th_params()
train_dfs  = ensemble3.load_train_features()
train_regs = ensemble3.load_train_regimes()

train_baseline_all     = ensemble3.compute_train_baseline(train_dfs, train_regs, ensemble3.MAIN_ALL_FEATS)
train_aux_baseline_all = ensemble3.compute_train_baseline(train_dfs, train_regs, aux_features)

hi_train_all = {}
fig, axes = plt.subplots(1, 4, figsize=(14, 3.5), sharey=True)
fig.suptitle("Exp F (A-full 3-Model) — Recalculated Train HI Trajectories (Global Train-baseline)", fontsize=13, fontweight="bold")

for ax, (b, color) in zip(axes, [(1, C["B1"]), (2, C["B2"]), (3, C["B3"]), (4, C["B4"])]):
    hi_corr, _, _, _ = ensemble3.compute_bearing_hi(
...
shutil.copy(
    BASE / "User/SR/0521/rul/output/th742_ensemble3/loocv_predictions.png",
...
shutil.copy(
    BASE / "User/SR/0521/rul/output/th742_ensemble3/test_predictions.png",
