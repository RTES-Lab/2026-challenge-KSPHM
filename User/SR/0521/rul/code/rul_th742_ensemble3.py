"""
User/SR/0521 — A-full 3-Model Ensemble (LGBM + LSTM-A + Transformer-A) Bearing RUL Pipeline
=========================================================================================
작업자: SR/0521
핵심 접근법:
  1. LGBM, LSTM-A, Transformer-A 세 모델의 예측 결과를 결합하는 3-Model Ensemble 파이프라인 구축.
  2. 세 모델이 서로 다른 아키텍처적 특성(Tabular, Recurrence, Attention)을 가지고 있어 에러가 상호 보완됨을 이용.
  3. LOOCV 시 dynamic LOO-baseline recalculation 구조를 적용하여 외삽 붕괴 방지.
  4. Test Inference 시 Train-baseline을 기준으로 HI 및 start_obs를 절대적 물리 궤적에 정렬.
  5. 세 모델의 폴드별 validation 스코어에 비례하여 동적으로 앙상블 가중치를 계산.
"""
...
# SR/0521 출력 디렉토리
OUT_DIR   = BASE / "User/SR/0521/rul/output/th742_ensemble3"
...
def run_loocv(train_dfs, train_regs, params, sigma, feat_w, grp_w, aux_features, aux_w, aux_s):
    ...
    with open(OUT_DIR / "loocv_log.txt", "w", encoding="utf-8") as f:
        f.write("SR/0521 — 3-Model Ensemble (LGBM+LSTM-A+Transformer-A) Bearing RUL Pipeline\n")
        ...
        for b in BEARINGS:
            r = results[b]
            f.write(f"  B{b}: LGBM={r['sc_lgbm']:.4f}  LSTM-A={r['sc_lstm']:.4f}  Transformer-A={r['sc_tf']:.4f}  Ens={r['sc_ens']:.4f}\n")

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("SR/0521 3-Model Ensemble LOOCV — Recalculated LOO-baseline HI", fontsize=12)
    for i, test_bid in enumerate(BEARINGS):
        ...
        ax.plot(obs, r["lstm"],  "g--", lw=1, alpha=0.7, label="LSTM-A")
        ax.plot(obs, r["transformer"],"c--", lw=1, alpha=0.7, label="Transformer-A")
...
def run_test_inference(hi_train_all, results, best_cf,
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n{'='*60}")
    print(f"  Test inference — 3-Model Ensemble, cf={best_cf:.2f}")
    ...
    fig.suptitle(f"SR/0521 Test RUL — 3-Model Ensemble  cf={best_cf:.2f}", fontsize=12)
    ...
    for i, tid in enumerate(TEST_IDS):
        ...
        print(f"  → 최종 RUL = {final_hr:.2f}hr  (참고용: afull={REF['afull'][tid]:.2f}, 0514={REF['0514'][tid]:.2f})\n")
        ...
        ax  = axes.flatten()[i]
        ax2 = ax.twinx()
        ax.plot(obs_pts, preds_lgbm,  "r--", lw=1, alpha=0.5, label="LGBM")
        ax.plot(obs_pts, preds_lstm,  "g--", lw=1, alpha=0.5, label="LSTM-A")
        ax.plot(obs_pts, preds_tf,    "c--", lw=1, alpha=0.5, label="Transformer-A")
        ax.plot(obs_pts, preds_final, "m-",  lw=2,             label="3-Model Ens")
    ...
    print(f"\n  {'Test':>5} | {'0514':>6} | {'afull':>6} | {'ens3_hours':>10} | start_obs | corr_start | corr_end")
    print(f"  {'-'*90}")
    for row in summary_rows:
        print(f"  {row['test_id']:>5} | {row['base0514']:>6.2f} | "
              f"{row['afull_hours']:>6.2f} | {row['rul_hours']:>10.2f} | "
...
if __name__ == "__main__":
    print("=" * 60)
    print("SR/0521 — 3-Model Ensemble (LGBM+LSTM+Transformer) A-full Pipeline")
    print("=" * 60)
