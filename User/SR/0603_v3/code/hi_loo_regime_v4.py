"""
HI LOO Regime v4 — v2 + log1p transform (energy/power/rms)
============================================================
v2 대비 변경:
  1. ch3_energy, ch3_total_power, ch3_rms → log1p 적용
     (step function → gradual trend, 정상기 미세 변화 살림)
  Kurtosis/CF 제외: ablation 결과 B3 Q 0.804→0.358 붕괴 확인
"""
...
OUT_TRAIN = os.path.join(SR_BASE, "output/train_v4")
OUT_TEST  = os.path.join(SR_BASE, "output/test_v4")
...
def _extract_one(fpath: str) -> dict:
    ...
    for ch_name in ["CH3", "CH4"]:
        ...
        energy_raw = float(np.sum(x ** 2))

        feat[f"{ch}_rms"]         = float(np.log1p(rms_raw))
        feat[f"{ch}_p2p"]         = float(np.max(x) - np.min(x))
        feat[f"{ch}_std"]         = float(np.std(x))
        feat[f"{ch}_energy"]      = float(np.log1p(energy_raw))

        nperseg = min(4096, len(x) // 8)
        fv, psd = welch(x, fs=FS, nperseg=nperseg)
        tp = float(np.sum(psd))
        feat[f"{ch}_total_power"] = float(np.log1p(tp))
...
def main():
    print("=" * 70)
    print("  HI LOO Regime v4 — log1p + Kurtosis/Crest Factor")
    print("=" * 70)
    ...
    fig_tr.suptitle("Train LOO Regime HI (v4 — log1p + Kurtosis/CF)", fontsize=13, fontweight="bold")
    ...
    for i, bid in enumerate(BEARINGS):
        ...
        # per-feature Q-score 출력 (비교용)
        rs_feat_q = rs[0]["feat_q"]  # regime 0 기준
        print(f"    [feat_q regime=0] " +
              "  ".join(f"{f.replace('ch3_','').replace('ch4_','ch4-')}={v:.3f}"
                        for f, v in rs_feat_q.items()))
    ...
    # v2 비교
    v2_qs = [0.573, 0.639, 0.804, 0.501]
    print(f"  v2 avg Q:    {np.mean(v2_qs):.4f}")
    for i, bid in enumerate(BEARINGS):
        row = df_train_sum[df_train_sum["bearing"] == bid].iloc[0]
        diff = row["q_score"] - v2_qs[i]
        print(f"    B{bid}: v4={row['q_score']:.3f}  v2={v2_qs[i]:.3f}  Δ={diff:+.3f}")
    ...
    fig_te.suptitle("Test Regime HI (v4 — log1p + Kurtosis/CF)",
    ...
    for i, tid in enumerate(TEST_IDS):
        ...
        tred = trendability(hi)
        q    = (mon + tred) / 2
        print(f"    n={n}  start={hi[0]:.3f}  end={hi[-1]:.3f}  Q={q:.4f}")
    ...
    print("=" * 70)
    print("\nTrain LOO:")
    print(df_train_sum[["bearing", "hi_start", "hi_end", "q_score"]].to_string(index=False))
