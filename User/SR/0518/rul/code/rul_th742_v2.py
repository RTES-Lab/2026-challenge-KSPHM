"""
SR/0518 v2 — TH v7_4_2 HI + SR RUL Pipeline (LGBM feature fix)
=================================================================
v1 대비 변경:
  LGBM 피처: raw window → window-minmax(trend shape) + raw 요약(절대 수준)
  목적: B3 HI 최댓값이 0.576으로 낮아 LGBM이 'HI=0.5 ≈ 초반'으로 오판하는 문제 해결

v1 결과 (baseline for comparison):
  LOOCV: LGBM=0.3625  LSTM-A=0.4261  Ensemble=0.4529  (SR 0514: 0.4326)
  B3 LGBM: 0.1344 (붕괴)
"""
...
OUT_DIR   = Path("/data/home/ksphm/2026-challenge-KSPHM/User/SR/0518/rul/output/th742_v2")
...
# ── LightGBM (v2: window-minmax + raw summary) ────────────────────────────
def make_lgbm_features(hi_array):
    """
    v1 (raw window only) → v2 (normalized window + raw summary)

    피처 구성 (16개):
      [0:10]  window_norm  — window-minmax 정규화 (trend shape, 스케일 무관)
      [10]    slope_norm   — 정규화 window 기울기
      [11]    hi_last      — 마지막 raw HI (절대 열화 수준)
      [12]    hi_mean      — window raw 평균 (절대 위치)
      [13]    hi_max       — window raw 최댓값 (절대 피크)
      [14]    slope_raw    — raw window 기울기 (절대 변화 속도)
      [15]    delta_raw    — window raw 첫-마지막 차이
    """
    features, targets = [], []
    N = len(hi_array)
    for i in range(SEQ_LENGTH, N):
        ...
        # Normalized window (scale-invariant trend shape)
        ...
        # Raw summary (absolute degradation level)
        slope_raw    = float(np.polyfit(np.arange(SEQ_LENGTH), window, 1)[0])

        feats = (list(window_norm) +
                 [slope_norm,
                  float(window[-1]),           # hi_last (raw)
                  float(window.mean()),        # hi_mean (raw)
                  float(window.max()),         # hi_max  (raw)
                  slope_raw,
                  float(window[-1] - window[0])])  # delta raw
...
def run_loocv(hi_train):
    ...
    print(f"\n  ┌───────────────────────────────────────────────────────────────┐")
    print(f"  │  Config          │  v1 LOOCV │  v2 LOOCV │ v2 cf │ v2 after cf │")
    print(f"  ├───────────────────────────────────────────────────────────────┤")
    print(f"  │  SR 0514 base    │  0.4326   │     —     │  0.76 │  (ref)      │")
    print(f"  │  LSTM-A          │  {V1['lstm']:.4f}  │  {np.mean(sc_lstm_list):.4f}  │  {cf_lstm:.2f} │  {sc_lstm_cf:.4f}     │")
    print(f"  │  LGBM+LSTM-A     │  {V1['ens']:.4f}  │  {np.mean(ens_scores):.4f}  │  {cf_ens:.2f} │  {sc_ens_cf:.4f}     │")
    print(f"  └───────────────────────────────────────────────────────────────┘")

    # Log
    with open(OUT_DIR / "loocv_log.txt", "w", encoding="utf-8") as f:
        f.write("SR/0518 v2 — TH v7_4_2 HI + LGBM(window-norm) + LSTM-A\n")
        f.write(f"LGBM avg:    {np.mean(sc_lgbm_list):.4f}  (v1: {V1['lgbm']:.4f})\n")
        f.write(f"LSTM-A avg:  {np.mean(sc_lstm_list):.4f}  (v1: {V1['lstm']:.4f})\n")
        f.write(f"Ensemble:    {np.mean(ens_scores):.4f}  (v1: {V1['ens']:.4f})\n")
        f.write(f"After cf:    LSTM-A cf={cf_lstm:.2f}→{sc_lstm_cf:.4f}  "
                f"Ensemble cf={cf_ens:.2f}→{sc_ens_cf:.4f}\n\n")
        f.write("Fold detail:\n")
        for b in BEARINGS:
            r = results[b]
    ...
    fig.suptitle("SR/0518 v2 LOOCV — TH v7_4_2 HI + LGBM(window-norm) + LSTM-A", fontsize=11)
...
def run_test_inference(hi_train, hi_test, results, best_config, best_cf):
    ...
    BASELINE_V1 = {1: 8.13, 2: 10.89, 3: 7.44, 4: 6.52, 5: 9.94, 6: 8.61}
    BASELINE_0514 = {1: 5.05, 2: 5.08, 3: 4.69, 4: 3.43, 5: 7.46, 6: 5.40}

    summary_rows = []
    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle(f"SR/0518 v2 Test RUL — TH v7_4_2 HI + LGBM(window-norm)  "
                 f"cf={best_cf:.2f}", fontsize=11)
    ...
    for i, tid in enumerate(TEST_IDS):
        ...
        print(f"  [Test{tid}] {final_hr:.2f}hr  "
              f"(v1={BASELINE_V1[tid]:.2f}hr, 0514={BASELINE_0514[tid]:.2f}hr)")
        ...
        summary_rows.append({
            ...
            "rul_cycles": round(final_cyc, 2),
            "rul_hours":  round(final_hr, 2),
            "v1_hours":   BASELINE_V1[tid],
            "base0514":   BASELINE_0514[tid],
        })
    ...
    print(f"\n  {'Test':>5} | {'0514':>8} | {'v1':>8} | {'v2':>8} | hi_start→end")
    print(f"  {'-'*55}")
    for row in summary_rows:
        print(f"  {row['test_id']:>5} | {row['base0514']:>8.2f} | "
              f"{row['v1_hours']:>8.2f} | {row['rul_hours']:>8.2f} | "
              f"{row['hi_start']:.3f}→{row['hi_end']:.3f}")
