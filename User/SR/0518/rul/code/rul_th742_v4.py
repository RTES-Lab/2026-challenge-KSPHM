"""
SR/0518 v4 — TH v7_4_2 HI + SR RUL Pipeline (LGBM + obs_fraction + start_obs 추정)
=====================================================================================
v3 대비 변경:
  Test inference 시 start_obs=0 고정 → Train HI 기반 역산으로 교체
  방법: Test 베어링의 hi_start 값이 Train 각 베어링 궤적에서 처음 등장하는
        cycle 위치를 찾아 평균 → start_obs 결정
  목적: Test 베어링의 실제 수명 위치를 반영해 obs_frac 편향 제거

v3 결과 (baseline):
  LOOCV: LGBM=0.4102  LSTM-A=0.4261  Ensemble=0.5004
  Test 문제: start_obs=0 → LGBM이 Test 베어링을 수명 초반으로 오판
"""
...
OUT_DIR   = Path("/data/home/ksphm/2026-challenge-KSPHM/User/SR/0518/rul/output/th742_v4")
...
def estimate_start_obs(hi_train, hi_target_start, train_bids=None):
    """
    Test 베어링의 첫 HI 값(hi_target_start)이 Train 궤적에서
    처음으로 그 값에 도달하는 cycle을 찾아 평균 반환.

    방법:
      각 Train bearing HI 배열에서 hi_target_start 이상인 첫 번째 index를
      찾고, 4개(또는 지정된) 베어링의 평균을 start_obs로 사용.
    """
    ...
    for b in train_bids:
        hi = hi_train[b]
        # hi_target_start 이상인 첫 index
        idx = np.argmax(hi >= hi_target_start)
        # argmax는 조건 미충족 시 0 반환 → 실제로 도달했는지 확인
        if hi[idx] >= hi_target_start:
            positions.append(int(idx))
        else:
            # Train에서 hi_target_start에 도달 못한 경우 → 마지막 index 사용
...
def run_test_inference(hi_train, hi_test, results, best_config, best_cf):
    ...
    fig.suptitle(f"SR/0518 v4 Test RUL — TH v7_4_2 HI + LGBM(+obs_frac+start_est)  cf={best_cf:.2f}",
                 fontsize=11)
    ...
    for i, tid in enumerate(TEST_IDS):
        ...
        # ── start_obs 추정 ─────────────────────────────────────────────
        hi_start_val = float(hi_t[0])
        start_obs, positions = estimate_start_obs(hi_train, hi_start_val)
        print(f"  [Test{tid}] hi_start={hi_start_val:.3f} → "
              f"start_obs={start_obs} (per-bearing: {positions})")

        preds_lstm = predict_lstm(lstm_models, rul_scale, hi_t,
                                  start_obs=start_obs, device=device)
        preds_lgbm = predict_lgbm(lgbm_model, hi_t, start_obs=start_obs)
        ...
        print(f"  [Test{tid}] {final_hr:.2f}hr  "
              f"(v3=?, v2={REF['v2'][tid]:.2f}, v1={REF['v1'][tid]:.2f}, 0514={REF['0514'][tid]:.2f})")
        ...
        ax.set_title(f"Test{tid}  HI={hi_t[0]:.3f}→{hi_t[-1]:.3f}  "
                     f"start={start_obs}  RUL={final_hr:.1f}hr")
        ax.set_xlabel("Observation cycle"); ax.set_ylabel("RUL (cycles)")
        ax.legend(fontsize=7); ax.grid(True, alpha=0.4)
    ...
    print(f"\n  {'Test':>5} | {'0514':>6} | {'v1':>6} | {'v2':>6} | {'v4':>6} | start | obs_frac0 | hi_end")
    print(f"  {'-'*70}")
    for row in summary_rows:
        print(f"  {row['test_id']:>5} | {row['base0514']:>6.2f} | "
              f"{row['v1_hours']:>6.2f} | {row['v2_hours']:>6.2f} | "
              f"{row['rul_hours']:>6.2f} | "
              f"{row['start_obs']:>5} | {row['obs_frac0']:>9.3f} | {row['hi_end']:.3f}")
