"""
HI LOO Regime v1 (0604) — CH1/CH2 kurtosis + crest factor 추가
================================================================
v2 (0603_v3) 대비 변경:
  1. CH1, CH2 kurtosis (log-transformed) 및 crest factor 추가
     → B3의 충격성(impulsive) 열화를 포착
  2. FEATURE_GROUPS에 "impulse" 그룹 추가
  3. 출력 경로 0604로 변경
"""
...
# 새 피처에 대한 fallback Q-score (기존 피처는 0603_v3 기준 유지)
FEATURE_Q_GLOBAL = {
...
def _extract_one(fpath: str) -> dict:
    ...
    # CH3, CH4: 기존 피처 (energy, freq domain)
    ...
    # CH1, CH2: kurtosis (log) + crest factor
    for ch_name in ["CH1", "CH2"]:
        x  = grp[ch_name][:]
        ch = ch_name.lower()
        rms = float(np.sqrt(np.mean(x ** 2)))
        k   = float(scipy_kurtosis(x))          # excess kurtosis (Fisher)
        feat[f"{ch}_kurt_log"] = float(np.log1p(max(k, 0.0)))
        feat[f"{ch}_crest"]    = float(np.max(np.abs(x)) / (rms + 1e-12))
...
def main():
    print("=" * 70)
    print("  HI LOO Regime v1 (0604) — +CH1/CH2 kurtosis + crest factor")
    print("=" * 70)
    ...
    fig_tr.suptitle("Train LOO Regime HI (0604 — +kurtosis/crest)", fontsize=13, fontweight="bold")
    ...
    for i, bid in enumerate(BEARINGS):
        ...
        for regime in [0, 1]:
            ...
            fq = rs[regime]["feat_q"]
            print(f"    [{lbl}] impulse Q: ch1_kurt_log={fq['ch1_kurt_log']:.3f}  "
                  f"ch2_kurt_log={fq['ch2_kurt_log']:.3f}  "
                  f"ch1_crest={fq['ch1_crest']:.3f}  ch2_crest={fq['ch2_crest']:.3f}")
    ...
    fig_te.suptitle("Test Regime HI (0604 — +kurtosis/crest)", fontsize=13, fontweight="bold")
