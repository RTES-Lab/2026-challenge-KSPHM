"""
RUL Regime v3: LOO-correct RUL — 위치 추정 포함
================================================================
HI source: SR/0603_v3/output/train/BearingX_HI.csv
           SR/0603_v3/output/test/TestX_HI.csv

v2 대비 핵심 수정:
  [LGBM] elapsed_frac 피처 추가 (현재 관측이 수명 어느 시점인지)
         Test: HI 초기값으로 train 궤적 매칭 → 시작 위치 추정
  [LSTM] window-local minmax → LOO train 기반 global standardization
         Test: estimate_start_frac으로 start_obs 추정
         (start_obs=0 가정은 이미 열화된 validation 데이터에 잘못된 위치 정보를 줌)
...
def make_lgbm_features(hi_arr: np.ndarray, regime_arr: np.ndarray,
    ...
    피처: HI window (seq_len개) + slope, mean, std, max, last, delta,
          ...
          elapsed_frac (NEW: 현재 관측이 수명 어느 시점인지)
    start_frac: 관측 시작 시점의 수명 비율
                Train/LOOCV → 0.0 (수명 처음부터 관측)
                Test        → estimate_start_frac으로 추정
    """
    feats, targets = [], []
    N = len(hi_arr)
...
def run_loocv():
    print("=" * 65)
    print("  RUL Regime v3 — LGBM(+elapsed_frac) + LSTM(global norm)")
    print("=" * 65)
    ...
    with open(OUT_DIR / "loocv_log.txt", "w") as f:
        f.write("RUL Regime v3 — LGBM(+elapsed_frac) + LSTM(global norm)\n")
    ...
    fig.suptitle("RUL Regime v3 — LOOCV (elapsed_frac + global norm)", fontsize=12)
    axes = axes.flatten()
...
def run_test_inference(loocv_results: dict):
    ...
    fig.suptitle(f"RUL Regime v3 — Test (cf={cf_test:.2f})", fontsize=12)
    axes = axes.flatten()
