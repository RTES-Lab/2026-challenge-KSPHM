"""
RUL composite_q v1 (0605) — Stage-Blended HI + honest LOOCV
=============================================================
Differences from rul_v1.py:

1. HI source: output_cq/ (stage-blended, kurtosis late-track)
   - train/: self-baseline + T_actual  (LSTM training data)
   - loocv/: fleet-baseline + T_estimated  (honest LOO test-fold simulation)
   - test/:  fleet-baseline + T_estimated  (test inference)

2. LOOCV: test fold uses loocv/ HI (fleet baseline, T_estimated),
   and start_frac estimated from HI level — identical to real test conditions.
   Previous approach used actual cycle position (optimistic).
"""
...
HI_TRAIN  = BASE / "User/SR/0605/output_cq/train"
HI_LOOCV  = BASE / "User/SR/0605/output_cq/loocv"
HI_TEST   = BASE / "User/SR/0605/output_cq/test"
OUT_DIR   = BASE / "User/SR/0605/output_cq/rul"
...
def load_loocv_hi():
    """Fleet-baseline + T_estimated HI for honest LOOCV evaluation."""
    data = {}
    for bid in BEARINGS:
        df = pd.read_csv(HI_LOOCV / f"Bearing{bid}_HI.csv")
        data[bid] = {"hi": df["HI"].values.astype(float),
                     "regime": df["regime"].values.astype(int)}
    return data
...
# ══════════════════════════════════════════════════════════════════
def run_sliding_loocv(train_data, loocv_data, device):
    """
    Honest LOOCV: test fold uses loocv_data (fleet-baseline + T_estimated).
    start_frac estimated from HI level, same as real test inference.
    """
    print("\n[2/4] Sliding Window LOOCV — LSTM (honest: fleet-baseline + HI-level sf)")
    ...
    for test_bid in BEARINGS:
        ...
        # LOOCV test fold: fleet-baseline HI (honest test simulation)
        hi_t        = loocv_data[test_bid]["hi"]
        reg_t       = loocv_data[test_bid]["regime"]
        N           = len(hi_t)
        ...
        # Sliding window — estimate start_frac from HI level (honest: unknown cycle pos)
        ...
        for t_start in range(0, N - WIN_SIZE + 1, STRIDE):
            ...
            # Estimate start_frac from HI level (same as test inference)
            sf = estimate_start_frac_hi(float(hi_win[0]), train_data, train_bids)
    ...
    fig.suptitle(f"Train — Sliding RUL (LSTM, composite_q)  avg={mean_sc:.3f}",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def run_test(train_data, test_data, device):
    ...
    fig_hi.suptitle("Test Bearings — Stage-Blended Fleet-Norm HI (composite_q)",
                    fontsize=13, fontweight="bold")
    ...
    fig_rul.suptitle("Test Bearings — RUL (LSTM, composite_q stage-blend HI)",
                     fontsize=13, fontweight="bold")
    plt.tight_layout()
...
if __name__ == "__main__":
    print("=" * 70)
    print("  RUL composite_q v1 — Stage-Blended HI + Honest LOOCV")
    print("=" * 70)
    ...
    loocv_data = load_loocv_hi()
    test_data  = load_test_hi()

    # 1. Train HI
    plot_train_hi(train_data)

    # 2+3. Honest Sliding LOOCV (loocv/ HI, HI-level start_frac)
    fold_results, mean_sc = run_sliding_loocv(train_data, loocv_data, device)
    ...
    print(f"\n{'='*70}")
    print(f"  Done")
    print(f"  Sliding LOOCV LSTM avg (honest): {mean_sc:.4f}")
