"""
TH v8_3 Model Zoo — SP V10c HI 입력 버전
==========================================
TH의 모델 구조(LGBM+LSTM+GRU+TCN+DTW 앙상블)에
SP V10c HI를 입력으로 사용.
"""
...
SP      = Path("/data/home/ksphm/2026-challenge-KSPHM/User/SP")
HI_DIR  = SP / "05-26/V1b" / "output"
OUT_DIR = SP / "05-26/V1b" / "output" / "rul_th"
...
def run_loocv(hi_train):
    ...
    # 전체 fold 평균으로 단일 가중치 계산 (test와 동일한 조건)
    avg_scores = {m: float(np.mean([results[b]["raw_scores"][m] for b in BEARINGS]))
                  for m in model_names}
    shared_weights = score_weights(avg_scores)
    print(f"\n[Shared weights from avg scores]")
    print("  " + "  ".join(f"{m}={avg_scores[m]:.4f}" for m in model_names))
    print(f"  weights: {{{', '.join(f'{k}:{v:.3f}' for k,v in shared_weights.items())}}}")
    ...
    print(f"final_weights: {{{', '.join(f'{k}:{v:.3f}' for k,v in final_weights.items())}}}")

    # 결과 저장
    rows = []
    for b in BEARINGS:
        r  = results[b]
        sc = score_curve(r["N"], r["obs_pts"], r["final"])
        er = error_summary(r["N"], r["obs_pts"], r["final"])
        rows.append({"test_bearing": b, "score": round(sc, 4),
                     "mean_er": round(er["mean_er"], 4)})
        results[b]["sc_final"] = sc
    rul_df = pd.DataFrame(rows)
    rul_df.to_csv(OUT_DIR.parent / "rul_results.csv", index=False)

    print(f"\n{'─'*50}")
    for b in BEARINGS:
        r = results[b]
        print(f"  B{b}: final={r['sc_final']:.4f}  "
              + "  ".join(f"{m}={r['raw_scores'][m]:.4f}" for m in model_names))
    print(f"  avg: {np.mean([results[b]['sc_final'] for b in BEARINGS]):.4f}")

    # 로그
    with open(OUT_DIR / "loocv_log.txt", "w", encoding="utf-8") as f:
        f.write("TH Model Zoo — SP V10c HI\n")
        f.write(cf_df.to_string(index=False) + "\n\n")
        f.write(f"zoo_avg: cf={zoo_avg_cf:.2f}, score={zoo_avg_sc:.4f}\n")
        f.write(f"zoo_cal: cf={zoo_cal_cf:.2f}, score={zoo_cal_sc:.4f}\n")
        f.write(f"Final: {final_kind}, cf={final_cf:.2f}, score={final_score:.4f}\n\n")
        for b in BEARINGS:
            r = results[b]
            f.write(f"B{b}: final={r['sc_final']:.4f}  "
                    + "  ".join(f"{m}={r['raw_scores'][m]:.4f}" for m in model_names) + "\n")
        f.write(f"avg: {np.mean([results[b]['sc_final'] for b in BEARINGS]):.4f}\n")

    # 플롯
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("TH Zoo — SP V10c HI LOOCV", fontsize=12)
    for ax, b in zip(axes.flatten(), BEARINGS):
        r   = results[b]
        obs = r["obs_pts"]
        ax.plot(obs, true_rul(r["N"], obs), "k-", lw=2, label="true")
        for m in model_names:
            ax.plot(obs, r[m], lw=0.9, alpha=0.5, label=m)
        ax.plot(obs, r["final"], "b-", lw=2, label="final")
        ax.set_title(f"B{b} final={r['sc_final']:.3f}")
...
def run_test(hi_train, hi_test, cal_cfs, final_weights, shared_weights, final_kind, final_cf):
    ...
    fig.suptitle(f"TH Zoo — SP V10c HI  Test RUL  ({final_kind} cf={final_cf:.2f})", fontsize=12)
    ...
    for ax, tid in zip(axes.flatten(), TEST_IDS):
        ...
        pd.DataFrame({"obs_cycle": obs, "rul_lgbm": p_lgbm, "rul_final": final,
                      "rul_hours": hours}).to_csv(OUT_DIR / f"Test{tid}_RUL.csv", index=False)
        summary_rows.append({"test_id": tid, "hi_start": round(float(hi[0]), 3),
                              "hi_end": round(float(hi[-1]), 3), "start_obs": start_obs,
        ...
        ax.set_title(f"T{tid}  hi={hi[0]:.2f}→{hi[-1]:.2f}  start={start_obs}  RUL={hours[-1]:.1f}hr")
        ax.set_xlabel("obs cycle"); ax.set_ylabel("RUL cycles")
        ax.legend(fontsize=7); ax.grid(True, alpha=0.3)
    ...
    plt.close()
    pd.DataFrame(summary_rows).to_csv(OUT_DIR / "test_summary.csv", index=False)

# ── main ──────────────────────────────────────────────────────────────────────
def main():
    print("=== TH Zoo — SP V10c HI ===\n")
