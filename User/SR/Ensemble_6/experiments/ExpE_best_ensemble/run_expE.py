"""
Ensemble_6 / Exp-E: Best Ensemble (Combining All Insights)
===========================================================
Combining insights from Exp-A, B, D:

Key findings:
  - Exp-D: DTW weight=0.85 (LGBM+TCN+DTW) → 0.5677 (best so far)
  - Exp-A: DTW=0.70 with all 5 models → 0.5555
  - DTW alone: 0.5499 (baseline)
  - LSTM/GRU consistently underperform LGBM/TCN → drop them

Strategy:
  1. Fine-tune DTW weight in 0.80-0.90 range (step 0.01)
  2. Try different LGBM/TCN ratios (currently 55/45)
  3. Multiple seeds for TCN stability
  4. Extended CF search range

This is the "best of breed" experiment combining all insights.
"""
...
TCN_SEEDS       = [42, 123, 777]   # 3 seeds for TCN stability
MIN_RUL_CYCLES  = 1.0
EPS             = 1e-8
...
# Fine-grained sweep around 0.85 optimum
DTW_FINE_SWEEP = np.arange(0.78, 0.93, 0.01).tolist()
# LGBM/TCN split within the (1-w_dtw) budget: [pure_lgbm, 3:2 split, 1:1 split, 2:3 split, pure_tcn]
LGBM_TCN_RATIOS = [
    (0.55, 0.45),   # current default (Exp-D)
    (0.65, 0.35),   # more LGBM
    (0.45, 0.55),   # more TCN
    (0.70, 0.30),   # strongly LGBM-weighted
]
...
def run_loocv(hi_train):
    ...
    # ── Fine-grained 2D sweep: DTW weight × LGBM/TCN ratio ───────────────────
    ...
    print(f"  Exp-D best (w_dtw=0.85, lgbm=0.55):  0.5677")
    print(f"  This experiment best:                {best_overall:.4f}")

    # Per-bearing final scores
    rows = []
    for b in BEARINGS:
    ...
    overall_final = float(np.mean([r["score"] for r in rows]))
    for r in rows:
        print(f"  B{r['bearing']}: score={r['score']:.4f}  mean_er={r['mean_er']:.2f}%")
    print(f"  Overall: {overall_final:.4f}")
    ...
    fig.suptitle(f"Exp-E Best LOOCV — w_dtw={w_dtw_best:.2f}  overall={overall_final:.4f}", fontsize=11)
    for ax, b in zip(axes.flatten(), BEARINGS):
        r = results[b]; obs = r["obs_pts"]; true = true_rul(r["N"], obs)
        ax.plot(obs, true, "k-", lw=2, label="True")
        ax.plot(obs, r["dtw_cal"] / cal_cfs["dtw"] * 0.68, "r--", lw=1.5, alpha=0.7,
                label="DTW×0.68")
        ax.plot(obs, r["final"], "b-", lw=2, label=f"Best ensemble")
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("Exp-E Train Er%", fontsize=11)
    for ax, b in zip(axes.flatten(), BEARINGS):
        r = results[b]; obs = r["obs_pts"]; true = true_rul(r["N"], obs)
    ...
    # Overall vs w_dtw for each LGBM ratio
    for r_lgbm, r_tcn in LGBM_TCN_RATIOS:
    ...
    axes[0].axhline(0.5677, color="r", ls="--", lw=0.8, label="Exp-D (0.5677)")
    axes[0].axhline(0.5499, color="k", ls="--", lw=0.8, label="Baseline (0.5499)")
    axes[0].axvline(w_dtw_best, color="b", ls=":", lw=0.8)
    axes[0].set_title("Overall vs DTW weight"); axes[0].set_xlabel("DTW weight")
    axes[0].set_ylabel("Overall score"); axes[0].legend(fontsize=7); axes[0].grid(True, alpha=0.3)

    # Per-bearing for best LGBM ratio
...
def run_test(hi_train, hi_test, cal_cfs, w_dtw, r_lgbm, r_tcn, cf_best):
    ...
    fig.suptitle(f"Exp-E Test — w_dtw={w_dtw:.2f}  cf={cf_best:.2f}", fontsize=11)
...
def save_hi_outputs(hi_train, hi_test):
    ...
    fig.suptitle("Exp-E Train HI", fontsize=11); plt.tight_layout()
    ...
    fig.suptitle("Exp-E Test HI", fontsize=11); plt.tight_layout()
    plt.savefig(OUT_DIR / "test_hi.png", dpi=150); plt.close()

def main():
    print("=== Ensemble_6 / Exp-E: Best Ensemble (Fine-tuned) ===\n")
