"""
User/SR/0520 — A-full Baseline & Align start_obs Bearing RUL Pipeline
===================================================================
작업자: SR/0520
핵심 접근법:
  1. LOOCV 시 own-baseline 대신 교차 베어링 기준(LOO-baseline)으로 검증 베어링까지 동적 재계산하여
     분포가 완벽하게 정렬된 환경에서 공정한 LOOCV 검색 및 평가 수행.
  2. Test Inference 시 4개 Train 베어링 전체의 정상 구간으로 계산된 Train-baseline으로 Test HI를 재계산.
  3. 테스트 베어링의 초기 HI 값(hi_start)을 Train HI 곡선과 비교 매칭하여 시작 시점(start_obs)을 추정.
     이로써 HI 수치와 obs_fraction 피처가 동일한 Degradation 수준 상에서 정렬(Alignment)되도록 함.
  4. 예측값은 물리적 상한선(참고용)으로 모니터링하며, 모든 파라미터 최적화는 오로지 LOOCV 검증 결과에 의존.
"""
...
# SR/0520 출력 디렉토리
OUT_DIR   = BASE / "User/SR/0520/rul/output/th742_afull"
...
# ── Gate + fusion (copied from TH v7_4_2) ─────────────────────
...
def estimate_start_obs(hi_test_start, hi_train_dict):
    ...
    for b, hi_tr in hi_train_dict.items():
        # train HI 중 hi_test_start 이상으로 최초 올라가는 시점
    ...
    return np.clip(start_obs, 0, 100)  # 무리한 추정을 제한하기 위한 안전 클램핑
...
def run_loocv(train_dfs, train_regs, params, sigma, feat_w, grp_w, aux_features, aux_w, aux_s):
    ...
    print(f"\n  Best config: {'LSTM-A' if best_config == 'lstm_a' else 'LGBM+LSTM-A'} (cf={best_cf:.2f})")

    # 결과 로그 파일 작성
    with open(OUT_DIR / "loocv_log.txt", "w", encoding="utf-8") as f:
        f.write("SR/0520 — A-full Baseline & Align start_obs Bearing RUL Pipeline\n")
        ...
        f.write(f"cf: LSTM-A cf={cf_lstm:.2f}→{sc_lstm_cf:.4f}  Ens cf={cf_ens:.2f}→{sc_ens_cf:.4f}\n\n")
        for b in BEARINGS:
            r = results[b]
            f.write(f"  B{b}: LGBM={r['sc_lgbm']:.4f}  LSTM-A={r['sc_lstm']:.4f}  Ens={r['sc_ens']:.4f}\n")

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("SR/0520 A-full LOOCV — Recalculated LOO-baseline HI", fontsize=12)
...
def run_test_inference(hi_train_all, results, best_config, best_cf,
    ...
    fig.suptitle(f"SR/0520 Test RUL — Train-baseline HI & Align start_obs  cf={best_cf:.2f}", fontsize=12)
    ...
    for i, tid in enumerate(TEST_IDS):
        ...
        preds_raw   = (w_lgbm * preds_lgbm + w_lstm * preds_lstm if best_config == "ens" else preds_lstm)
        preds_final = preds_raw * best_cf

        final_cyc = float(preds_final[-1])
        final_hr  = final_cyc * INTERVAL_SEC / 3600
        print(f"  → 최종 RUL = {final_hr:.2f}hr  (참고용: v4={REF['v4'][tid]:.2f}, 0514={REF['0514'][tid]:.2f})\n")
    ...
    print(f"\n  {'Test':>5} | {'0514':>6} | {'v4':>6} | {'v6_afull':>8} | start_obs | corr_start | corr_end")
    print(f"  {'-'*80}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("SR/0520 — A-full Baseline & Align start_obs RUL Pipeline")
    print("=" * 60)
