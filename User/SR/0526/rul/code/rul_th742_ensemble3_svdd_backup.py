"""
User/SR/0521 — A-full 3-Model Ensemble with SVDD-guided start_obs alignment
=============================================================================
작업자: SR/0521
핵심 접근법:
  1. LGBM, LSTM-A, Transformer-A 세 모델의 예측 결과를 결합하는 3-Model Ensemble 파이프라인 구축.
  2. global SVDD(One-Class SVM) 모델을 학습시켜 테스트 베어링의 물리적인 열화 시작 시점(FPT, t_d)을 강건하게 감지.
  3. start_obs를 start_obs = mean_train_fpt - t_d로 추정하여, domain baseline shift에 따른 start_obs 과소/과대 추정 오차를 원천 차단.
  4. LOOCV 시 dynamic LOO-baseline recalculation 구조를 적용하여 외삽 붕괴 방지.
"""
...
# ── Global SVDD Training & start_obs Estimation ───────────────────────────
def train_global_svdd(train_dfs):
    # Pool healthy features (first 50 cycles of each training bearing)
    healthy_features = []
    for b in BEARINGS:
        df = train_dfs[b]
        X = df[MAIN_ALL_FEATS].values
        healthy_features.append(X[:50])
...
def estimate_start_obs_svdd(feat_df, ocsvm, scaler, mean_train_fpt=80):
    X = feat_df[MAIN_ALL_FEATS].values
    X_scaled = scaler.transform(X)
    
    # Anomaly score is -decision_function
    scores = -ocsvm.decision_function(X_scaled)
    
    # Detect cycle of degradation onset (t_d)
    t_d = None
    for i in range(len(scores) - 2):
        if scores[i] > 0.2 and scores[i+1] > 0.2 and scores[i+2] > 0.2:
            t_d = i
            break
    ...
    start_obs = mean_train_fpt - t_d
    return int(np.clip(start_obs, 0, 100))

# ── Heuristic start_obs mapping (for reference/logging only) ────────────────
def estimate_start_obs_heuristic(hi_test_start, hi_train_dict):
    candidate_cycles = []
...
def run_loocv(train_dfs, train_regs, params, sigma, feat_w, grp_w, aux_features, aux_w, aux_s):
    ...
    results = {}
    sc_lgbm_list, sc_lstm_list, sc_tf_list = [], [], []
    ...
    for test_bid in BEARINGS:
        ...
        for b in BEARINGS:
            hi_corr, _, _, _ = compute_bearing_hi(
                train_dfs[b], train_regs[b], params,
                sigma, feat_w, grp_w, aux_features, aux_w, aux_s,
                train_baseline_loo, train_aux_baseline_loo)
            hi_train_loo[b] = hi_corr

        N_test = len(hi_train_loo[test_bid])
        obs_pts = np.arange(SEQ_LENGTH, N_test)

        # 2.1 LOO-SVDD 학습 및 Validation Bearing의 SVDD HI / Onset 계산
        healthy_features_loo = []
        for b in train_bids:
            df = train_dfs[b]
            healthy_features_loo.append(df[MAIN_ALL_FEATS].values[:50])
        X_healthy_loo = np.vstack(healthy_features_loo)
        scaler_loo = StandardScaler()
        X_healthy_loo_scaled = scaler_loo.fit_transform(X_healthy_loo)
        ocsvm_loo = OneClassSVM(nu=0.05, kernel='rbf', gamma='scale')
        ocsvm_loo.fit(X_healthy_loo_scaled)

        X_val = train_dfs[test_bid][MAIN_ALL_FEATS].values
        X_val_scaled = scaler_loo.transform(X_val)
        svdd_scores_loo = -ocsvm_loo.decision_function(X_val_scaled)

        # 3사이클 연속 초과 시점 t_d 탐지
        t_d_loo = None
        for i_c in range(len(svdd_scores_loo) - 2):
            if svdd_scores_loo[i_c] > 0.2 and svdd_scores_loo[i_c+1] > 0.2 and svdd_scores_loo[i_c+2] > 0.2:
                t_d_loo = i_c
                break
        if t_d_loo is None:
            t_d_loo = len(svdd_scores_loo)
        start_obs_svdd_loo = int(np.clip(80 - t_d_loo, 0, 100))

        # SVDD HI 계산 (정규화)
        svdd_hi_loo = (svdd_scores_loo - svdd_scores_loo.min()) / (svdd_scores_loo.max() - svdd_scores_loo.min() + 1e-8)
        svdd_hi_loo = ema_smooth(svdd_hi_loo)
        svdd_hi_loo = moving_average(svdd_hi_loo)

        # 3. LGBM, LSTM-A, Transformer-A 학습 (train_bids 기준)
        lgbm_model = train_lgbm(hi_train_loo, train_bids)
        preds_lgbm = predict_lgbm(lgbm_model, hi_train_loo[test_bid], start_obs=0)

        lstm_models, rul_scale_lstm = train_lstm_ensemble(hi_train_loo, train_bids, device)
        preds_lstm = predict_lstm(lstm_models, rul_scale_lstm, hi_train_loo[test_bid], start_obs=0, device=device)

        tf_models, rul_scale_tf = train_transformer_ensemble(hi_train_loo, train_bids, device)
        preds_tf = predict_transformer(tf_models, rul_scale_tf, hi_train_loo[test_bid], start_obs=0, device=device)

        # 3.1 SVDD-Aligned RUL 예측 계산 (start_obs = start_obs_svdd_loo)
        preds_lgbm_svdd = predict_lgbm(lgbm_model, hi_train_loo[test_bid], start_obs=start_obs_svdd_loo)
        preds_lstm_svdd = predict_lstm(lstm_models, rul_scale_lstm, hi_train_loo[test_bid], start_obs=start_obs_svdd_loo, device=device)
        preds_tf_svdd   = predict_transformer(tf_models, rul_scale_tf, hi_train_loo[test_bid], start_obs=start_obs_svdd_loo, device=device)

        def avg_score(preds):
            return float(np.nanmean([competition_score(N_test - obs, p)
                                     for obs, p in zip(obs_pts, preds)]))
        sc_l  = avg_score(preds_lgbm)
        sc_lm = avg_score(preds_lstm)
        sc_t  = avg_score(preds_tf)
        print(f"  → LGBM: {sc_l:.4f}  LSTM-A: {sc_lm:.4f}  Transformer-A: {sc_t:.4f}")
        sc_lgbm_list.append(sc_l)
        sc_lstm_list.append(sc_lm)
        sc_tf_list.append(sc_t)

        # 3-Model 동적 가중치 계산 (역에러/스코어 비례)
        w_l   = max(sc_l,  1e-4)
        w_lm  = max(sc_lm, 1e-4)
        w_tf  = max(sc_t,  1e-4)
        ...
        preds_ens = [w_l * l + w_lm * lm + w_tf * t for l, lm, t in zip(preds_lgbm, preds_lstm, preds_tf)]
        preds_ens_svdd = [w_l * l + w_lm * lm + w_tf * t for l, lm, t in zip(preds_lgbm_svdd, preds_lstm_svdd, preds_tf_svdd)]

        results[test_bid] = {
            "lgbm": list(preds_lgbm), "lstm": list(preds_lstm), "transformer": list(preds_tf), "ens": preds_ens,
            "ens_svdd": preds_ens_svdd,
            "obs_pts": list(obs_pts), "N": N_test,
            "sc_lgbm": sc_l, "sc_lstm": sc_lm, "sc_tf": sc_t,
            "w_lgbm": w_l, "w_lstm": w_lm, "w_tf": w_tf,
            "svdd_hi": list(svdd_hi_loo), "t_d": t_d_loo, "start_obs_svdd": start_obs_svdd_loo
        }

    ens_scores = []
    print(f"\n{'='*60}")
    print(f"  {'Bearing':>8} | {'LGBM':>8} | {'LSTM-A':>8} | {'Transformer':>8} | {'Ensemble':>8}")
    print(f"  {'-'*65}")
    for test_bid in BEARINGS:
        r  = results[test_bid]
        ...
        print(f"  {test_bid:>8} | {r['sc_lgbm']:>8.4f} | {r['sc_lstm']:>8.4f} | {r['sc_tf']:>8.4f} | {sc:>8.4f}")
    print(f"  {'Mean':>8} | {np.mean(sc_lgbm_list):>8.4f} | "
          f"{np.mean(sc_lstm_list):>8.4f} | {np.mean(sc_tf_list):>8.4f} | {np.mean(ens_scores):>8.4f}")

    print("\n  [Calibration search]")
    cf_ens, sc_ens_cf = search_calibration(
        {b: results[b]["ens"] for b in BEARINGS}, results, "3-Model Ensemble")

    best_cf = cf_ens
    print(f"\n  Best config: 3-Model Ensemble (cf={best_cf:.2f} → {sc_ens_cf:.4f})")

    # 결과 로그 파일 작성
    with open(OUT_DIR / "loocv_log.txt", "w", encoding="utf-8") as f:
        f.write("SR/0526 — 3-Model Ensemble (LGBM+LSTM-A+Transformer-A) with SVDD start_obs Alignment\n")
        f.write(f"LOOCV uses DYNAMIC LOO-baseline HI recalculation & SVDD monitoring\n\n")
        ...
        f.write(f"cf: Ens cf={cf_ens:.2f}→{sc_ens_cf:.4f}\n\n")
        for b in BEARINGS:
            r = results[b]
            f.write(f"  B{b}: LGBM={r['sc_lgbm']:.4f}  LSTM-A={r['sc_lstm']:.4f}  Transformer-A={r['sc_tf']:.4f}  Ens={r['sc_ens']:.4f}  SVDD start_obs={r['start_obs_svdd']}\n")

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("SR/0526 3-Model Ensemble LOOCV — Baseline vs SVDD-Aligned RUL Predictions", fontsize=12, fontweight="bold")
    for i, test_bid in enumerate(BEARINGS):
        ax  = axes.flatten()[i]
        r   = results[test_bid]
        obs = r["obs_pts"]
        
        # Plot pure RUL curves
        ax.plot(obs, [r["N"] - o for o in obs], "k-",  lw=2.5, label="True RUL")
        ax.plot(obs, r["ens"],   "g--",  lw=1.5, label="Baseline Ensemble (start_obs=0)")
        ax.plot(obs, r["ens_svdd"], "b-", lw=2.0, label=f"SVDD-Aligned Ensemble (start_obs={r['start_obs_svdd']})")
        
        ax.set_title(f"Bearing {test_bid} (SVDD start_obs={r['start_obs_svdd']})", fontsize=10, fontweight="bold")
    ...
    return results, best_cf, sc_ens_cf

# ── Test inference with Train-baseline corrected HI & SVDD start_obs Alignment ──
def run_test_inference(hi_train_all, results, best_cf,
                       test_dfs, test_regs, params, sigma, feat_w, grp_w,
                       aux_features, aux_w, aux_s,
                       train_baseline, train_aux_baseline, train_dfs):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n{'='*60}")
    print(f"  Test inference — 3-Model Ensemble with Global SVDD start_obs Alignment, cf={best_cf:.2f}")
    print(f"  HI mode: Train-baseline + SVDD start_obs\n")

    # 1. Train Global SVDD Model
    print("  Training Global SVDD (OneClassSVM) on training healthy data...")
    ocsvm, scaler = train_global_svdd(train_dfs)

    lgbm_model = train_lgbm(hi_train_all, BEARINGS)
    print("  LSTM training on all 4 bearings...")
    lstm_models, rul_scale_lstm = train_lstm_ensemble(hi_train_all, BEARINGS, device)
    print("  Transformer training on all 4 bearings...")
    tf_models, rul_scale_tf = train_transformer_ensemble(hi_train_all, BEARINGS, device)
    ...
    total  = w_lgbm + w_lstm + w_tf
    w_lgbm, w_lstm, w_tf = w_lgbm / total, w_lstm / total, w_tf / total
    print(f"  Ensemble Weights: LGBM={w_lgbm:.3f}, LSTM-A={w_lstm:.3f}, Transformer-A={w_tf:.3f}")
    ...
    fig.suptitle(f"SR/0526 Test Inference — SVDD Diagnostics & 3-Model Ensemble RUL Predictions (cf={best_cf:.2f})", fontsize=14, fontweight="bold")
    ...
    for i, tid in enumerate(TEST_IDS):
        ...
        # 2. SVDD start_obs 추정 및 full SVDD HI 계산
        X_t = feat_df[MAIN_ALL_FEATS].values
        X_t_scaled = scaler.transform(X_t)
        svdd_scores_t = -ocsvm.decision_function(X_t_scaled)
        
        t_d = None
        for i_c in range(len(svdd_scores_t) - 2):
            if svdd_scores_t[i_c] > 0.2 and svdd_scores_t[i_c+1] > 0.2 and svdd_scores_t[i_c+2] > 0.2:
                t_d = i_c
                break
        if t_d is None:
            t_d = len(svdd_scores_t)
            
        start_obs = int(np.clip(80 - t_d, 0, 100))
        obs_pts = np.arange(SEQ_LENGTH, len(hi_corr))
        
        # SVDD HI per test bearing min-max normalized
        svdd_hi_t = (svdd_scores_t - svdd_scores_t.min()) / (svdd_scores_t.max() - svdd_scores_t.min() + 1e-8)
        svdd_hi_t = ema_smooth(svdd_hi_t)
        svdd_hi_t = moving_average(svdd_hi_t)
        
        # Log comparison
        old_heuristic_start_obs = estimate_start_obs_heuristic(hi_corr[0], hi_train_all)
        print(f"  [Test{tid}] HI: [{hi_corr[0]:.4f} → {hi_corr[-1]:.4f}]")
        print(f"    SVDD start_obs: {start_obs} (Heuristic got {old_heuristic_start_obs})")

        # 3. 정렬된 start_obs 기반 RUL 예측
        preds_lstm  = predict_lstm(lstm_models, rul_scale_lstm, hi_corr, start_obs=start_obs, device=device)
        preds_tf    = predict_transformer(tf_models, rul_scale_tf, hi_corr, start_obs=start_obs, device=device)
        preds_lgbm  = predict_lgbm(lgbm_model, hi_corr, start_obs=start_obs)
        preds_raw   = (w_lgbm * preds_lgbm + w_lstm * preds_lstm + w_tf * preds_tf)
        ...
        # Left plot: SVDD HI & Onset
        ax_svdd = axes[i, 0]
        ax_svdd.plot(np.arange(len(svdd_hi_t)), svdd_hi_t, "b-", lw=2, label="SVDD HI")
        ax_svdd.plot(np.arange(len(hi_corr)), hi_corr, "k-", lw=1.5, alpha=0.5, label="Corrected HI (hi_corr)")
        ax_svdd.axhline(y=0.2, color="r", linestyle="--", lw=1.5, label="Threshold (0.2)")
        if t_d < len(svdd_hi_t):
            ...
            ax_svdd.axvspan(t_d, len(svdd_hi_t), color="salmon", alpha=0.15, label="Degraded Phase")
            
        ax_svdd.set_title(f"Test {tid} SVDD Diagnostic (Estimated start_obs = {start_obs})", fontsize=10, fontweight="bold")
        ...
        ax_rul = axes[i, 1]
        ax_rul.plot(obs_pts, preds_lgbm,  "r--", lw=1.2, alpha=0.5, label="LGBM")
        ax_rul.plot(obs_pts, preds_lstm,  "g--", lw=1.2, alpha=0.5, label="LSTM-A")
        ax_rul.plot(obs_pts, preds_tf,    "c--", lw=1.2, alpha=0.5, label="Transformer-A")
        ax_rul.plot(obs_pts, preds_final, "m-",  lw=2.5,             label="3-Model Ens")
    ...
    print(f"\n  {'Test':>5} | {'0514':>6} | {'afull':>6} | {'ens3':>6} | {'svdd_hours':>10} | start_obs | corr_start | corr_end")
    print(f"  {'-'*105}")
    ...
    with open(OUT_DIR / "loocv_log.txt", "a", encoding="utf-8") as f:
        f.write("\nTest inference (Train-baseline HI & SVDD start_obs):\n")
...
if __name__ == "__main__":
    print("=" * 60)
    print("SR/0521 — 3-Model Ensemble (LGBM+LSTM+Transformer) with Global SVDD start_obs")
    print("=" * 60)
    ...
    # 4. LOOCV 실행
    print("\nStarting LOOCV...")
    results, best_cf, sc_ens_cf = run_loocv(
    ...
    run_test_inference(
        hi_train_all, results, best_cf,
        test_dfs, test_regs, params, sigma, feat_w, grp_w,
        aux_features, aux_w, aux_s,
        train_baseline_all, train_aux_baseline_all, train_dfs)
